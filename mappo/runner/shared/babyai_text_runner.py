...
class BabyAITextRunner(VirtualHomeRunner):
    ...
    def run(self):
        ...
        total_num_steps = 0
        for episode in tqdm(range(episodes), desc="episodes", disable=not is_main_process()):
            ...
            if is_main_process() and episode % self.save_json_interval == 0:
    ...
    def collect_experiences(self, episode):
        ...
        games_done = 0
        for step in tqdm(range(self.episode_length), desc="steps", disable=not is_main_process()):
            # Sample actions
