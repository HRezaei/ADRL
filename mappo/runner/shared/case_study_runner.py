...
from mappo.utils.case_study_buffer import CriticBuffer
from mappo.trainers.critic_trainer_appo import CriticAPPOTrainer
from mappo.trainers.critic_trainer_tppo import CriticTPPOTrainer
import pickle
...
class CaseStudyRunner:
    ...
    def __init__(self, config):
        ...
        self.agent = LlamaCritic(self.all_args.model_name, self.all_args.max_new_tokens, self.algo)
        self.buffer = CriticBuffer(self.all_args, self.num_agents, self.agent.tokenizer.pad_token_id)
        

        if self.algo == "TWOSOME":
            self.trainer = CriticAPPOTrainer(self.all_args, self.agent, self.num_agents)
        elif self.algo in ["POAD", "NTPO"] :
            self.trainer = CriticTPPOTrainer(self.all_args, self.agent, self.num_agents)
        else:
    ...
    def run(self):
        ...
        for episode in range(episodes):
            for step in range(self.episode_length):
                # Sample actions
                values, actions, action_tokens = self.collect(step)
    ...
    def collect(self, step):
        ...
        actions, action_tokens, values = behaviour_data
        ...
        return values, actions, action_tokens

    def insert(self, data):
        obs, rewards, dones, ava, values, actions, action_tokens = data
        ...
        if self.algo == "TWOSOME":
            self.buffer.insert_appo(obs, ava, actions, values, rewards, masks, action_tokens)
        elif self.algo in ["POAD", "NTPO"]:
            self.buffer.insert_tppo(obs, ava, actions, values, rewards, masks, action_tokens)
        else:
            raise NotImplementedError
