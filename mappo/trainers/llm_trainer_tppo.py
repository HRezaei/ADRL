...
class TPPOTrainer:
    ...
    def ppo_update(self, sample):
        ...
        # pi_logits, _ = self.agent.infer_for_token_update(np.concatenate(obs_batch), 
        #                                                  np.concatenate(action_batch))
        # pi_logits = pi_logits.view(batch_size, -1, *pi_logits.shape[-2:])
        # pi_log_prob = torch.log_softmax(pi_logits, dim=-1)
        # log_prob_infer = torch.gather(pi_log_prob, -1, action_tokens_batch.unsqueeze(-1)).squeeze(-1)
        # entropy = Categorical(logits=pi_logits).entropy()
        
        # policy_loss, approx_kl = self.cal_policy_loss(log_prob_infer, log_prob_batch, advantages_batch, 
        #                                                 entropy, token_mask)
        # policy_loss.backward()
        # if approx_kl > 0.2:
        #     self.policy_optimizer.zero_grad()
